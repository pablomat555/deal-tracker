...
def _get_client() -> gspread.Client:
    ...
    if _gspread_client is None:
        try: _gspread_client = gspread.service_account(filename=config.GOOGLE_CREDS_JSON_PATH)
        except Exception as e: logger.critical(f"Критическая ошибка авторизации Google: {e}", exc_info=True); raise
...
def _get_sheet_by_name(sheet_name: str) -> Optional[gspread.Worksheet]:
    ...
    except gspread.exceptions.WorksheetNotFound: logger.error(f"Лист '{sheet_name}' не найден."); return None
...
def _parse_decimal(value: Any) -> Optional[Decimal]:
    ...
    try: return Decimal(str(value).replace(' ', '').replace('\u00A0', '').replace(',', '.'))
    except (InvalidOperation, TypeError): return None

def _format_decimal(value: Optional[Decimal]) -> str: return str(value).replace('.', ',') if value is not None else ""
def _format_datetime(value: Optional[datetime]) -> str: return value.strftime("%Y-%m-%d %H:%M:%S") if value is not None else ""
def _format_bool(value: Optional[bool]) -> str: return "TRUE" if value else "FALSE" if value is not None else ""
...
def _build_model_from_row(row: List[str], headers: List[str], model_cls: Type[T], row_num: int) -> T:
    ...
    for field_name, field_type in model_fields.items():
        ...
        try:
            ...
            if "Колонка для поля" in str(e) and is_optional: kwargs[field_name] = None; continue
...
def batch_get_records(sheets_to_fetch: Dict[str, Type[T]]) -> tuple[Dict[str, List[Any]], List[str]]:
    ...
    try:
        ...
        for sheet_name in sheet_names:
            if sheet_name not in value_ranges: all_errors.append(f"Лист '{sheet_name}' не был найден в ответе API."); continue
            all_values = value_ranges[sheet_name].get('values', [])
            if not all_values or len(all_values) < 2: logger.warning(f"Лист '{sheet_name}' пуст или содержит только заголовки."); continue
            ...
            for j, row_values in enumerate(data_rows):
                ...
                try:
                    instance = _build_model_from_row(row_values, headers, model_cls, j + 2)
                    if hasattr(instance, 'row_number'): instance.row_number = j + 2
                    records.append(instance)
                except Exception as e: all_errors.append(f"Лист '{sheet_name}', строка {j+2}: {e}")
            all_data[sheet_name] = records
    except Exception as e: error_msg = f"Критическая ошибка при пакетном чтении листов: {e}"; logger.error(error_msg, exc_info=True); all_errors.append(error_msg)
...
def append_record(sheet_name: str, record: Any) -> bool:
    try:
        sheet = _get_sheet_by_name(sheet_name); headers = _get_headers(sheet_name)
        if not sheet or not headers: return False
        sheet.append_row(_model_to_row(record, headers), value_input_option='USER_ENTERED'); invalidate_cache(sheet_name); return True
    except Exception as e: logger.error(f"Ошибка добавления в '{sheet_name}': {e}", exc_info=True); return False

def delete_row(sheet_name: str, row_number: int) -> bool:
    try:
        sheet = _get_sheet_by_name(sheet_name)
        if not sheet: return False
        sheet.delete_rows(row_number); invalidate_cache(sheet_name); return True
    except Exception as e: logger.error(f"Ошибка удаления строки {row_number} из '{sheet_name}': {e}", exc_info=True); return False
...
def update_position(position: PositionData) -> bool:
    ...
    try:
        sheet = _get_sheet_by_name(sheet_name); headers = _get_headers(sheet_name)
        if not sheet or not headers: return False
        row_values = _model_to_row(position, headers); range_str = f'A{position.row_number}:{chr(ord("A") + len(headers) - 1)}{position.row_number}'
        sheet.batch_update([{'range': range_str, 'values': [row_values]}], value_input_option='USER_ENTERED'); invalidate_cache(sheet_name); return True
    except Exception as e: logger.error(f"Ошибка обновления позиции {position.symbol}: {e}", exc_info=True); return False

def batch_update_positions(positions: List[PositionData]) -> bool:
    """[ВОССТАНОВЛЕНО] Пакетно обновляет несколько строк позиций."""
    ...
    try:
        ...
        logger.error(f"Ошибка пакетного обновления позиций: {e}", exc_info=True)
        return False

def batch_update_balances(changes: List[Dict[str, Any]]) -> bool:
    sheet_name = config.ACCOUNT_BALANCES_SHEET_NAME; sheet = _get_sheet_by_name(sheet_name)
    if not sheet: return False
    all_balances, errors = get_all_records(sheet_name, BalanceData)
    if errors: logger.error(f"Не удалось прочитать балансы перед обновлением: {errors}"); return False
    balances_map = {(b.account_name.lower(), b.asset.upper()): b for b in all_balances}
    to_update, to_add = [], []
    for change in changes:
        key = (change['account'].lower(), change['asset'].upper()); amount = change['change']
        if key in balances_map:
            b_obj = balances_map[key]; b_obj.balance = (b_obj.balance or Decimal(0)) + amount; b_obj.last_updated = datetime.now()
            if b_obj not in to_update: to_update.append(b_obj)
        else:
            new_b = BalanceData(account_name=key[0].capitalize(), asset=key[1], balance=amount, last_updated=datetime.now()); to_add.append(new_b); balances_map[key] = new_b
    try:
        headers = _get_headers(sheet_name)
        if to_update:
    ...
    except Exception as e: logger.critical(f"Критическая ошибка записи балансов: {e}", exc_info=True); return False
...
def batch_update_trades_fifo_fields(updates: List[Dict[str, Any]]) -> bool:
    ...
    try:
        ...
        for update in updates:
            ...
            if 'fifo_consumed_qty' in update: payload.append({'range': gspread.utils.rowcol_to_a1(row_num, consumed_qty_col), 'values': [[_format_decimal(update['fifo_consumed_qty'])]]})
            if 'fifo_sell_processed' in update: payload.append({'range': gspread.utils.rowcol_to_a1(row_num, processed_col), 'values': [[_format_bool(update['fifo_sell_processed'])]]})
        if not payload: return True
        sheet.batch_update(payload, value_input_option='USER_ENTERED'); invalidate_cache(sheet_name); return True
    except Exception as e: logger.error(f"Ошибка пакетного обновления FIFO полей: {e}", exc_info=True); return False

def batch_append_fifo_logs(fifo_logs: List[FifoLogData]) -> bool:
    if not fifo_logs: return True
    sheet_name = config.FIFO_LOG_SHEET_NAME
    try:
        sheet = _get_sheet_by_name(sheet_name)
        if not sheet: return False
        headers = _get_headers(sheet_name)
        if not headers: logger.error(f"Не удалось добавить FIFO логи: нет заголовков в листе '{sheet_name}'."); return False
        rows_to_append = [_model_to_row(log, headers) for log in fifo_logs]
        sheet.append_rows(rows_to_append, value_input_option='USER_ENTERED')
        invalidate_cache(sheet_name)
        return True
    except Exception as e: logger.error(f"Ошибка пакетного добавления в Fifo_Log: {e}", exc_info=True); return False
...
def update_system_status(status: str, timestamp: datetime) -> bool:
    ...
    try:
        sheet = _get_sheet_by_name(sheet_name);
        if not sheet: return False
        payload = [{'range': config.UPDATER_LAST_RUN_CELL, 'values': [[_format_datetime(timestamp)]]}, {'range': config.UPDATER_STATUS_CELL, 'values': [[status]]}]
        sheet.batch_update(payload, value_input_option='USER_ENTERED'); return True
    except Exception as e: logger.error(f"Ошибка обновления статуса системы: {e}", exc_info=True); return False

def update_or_create_analytics_record(analytics_data: AnalyticsData) -> bool:
    sheet_name = config.ANALYTICS_SHEET_NAME
    try:
        sheet = _get_sheet_by_name(sheet_name)
        if not sheet: return False
        headers = _get_headers(sheet_name)
        if not headers: logger.error(f"Не удалось обновить '{sheet_name}': нет заголовков."); return False
        row_values = _model_to_row(analytics_data, headers)
        if sheet.row_count > 1:
            range_to_update = f'A2:{gspread.utils.rowcol_to_a1(2, len(headers))}'
            sheet.update(range_to_update, [row_values], value_input_option='USER_ENTERED')
        else:
...
def get_all_fund_movements() -> List[MovementData]:
    records, errors = get_all_records(config.FUND_MOVEMENTS_SHEET_NAME, MovementData)
    if errors: logger.error(f"Ошибки при чтении Fund_Movements: {errors}")
    return records

def get_all_fifo_logs() -> List[FifoLogData]:
    records, errors = get_all_records(config.FIFO_LOG_SHEET_NAME, FifoLogData)
    if errors: logger.error(f"Ошибки при чтении Fifo_Log: {errors}")
    return records